...
║    Both saved by gold_miner_trainer.py in xle_output/                    ║
║                                                                              ║
║  STRATEGY                                                                   ║
...
║    shel_signal_chart.png     — candlestick chart with buy/sell markers      ║
...
BCFG = {
    # ── Signal thresholds ─────────────────────────────────────────────────
    # XLE predictions are moderate magnitude — use 0.003 threshold
...
def run_backtest(pred_df, bcfg, verbose=True):
    ...
    if trend_on:
        try:
            ...
            gdx_hist = yf.Ticker("SHEL").history(
...
def plot_candle_signal(results_df, pred_df, lookback_years=2):
    ...
    ohlcv      = ticker.history(start=start_date, end=end_date,
                                auto_adjust=True)
    if len(ohlcv) < 10:
        print(f"  ✗ Could not download SHEL OHLCV data")
        return
    ...
    path = os.path.join(BACKTEST_DIR, "shel_signal_chart.png")
    plt.savefig(path, dpi=150, bbox_inches="tight", facecolor=DARK)
    plt.close()
...
def main():
    ...
    # 1. Load predictions
    # print("DEBUG: about to call load_oof_predictions", flush=True)
    oof_df = load_oof_predictions()
    # print(f"DEBUG: load returned type={type(oof_df).__name__}", flush=True)
    if oof_df is None:
        return
    ...
    print(f"    shel_signal_chart.png     — candle chart with buy/sell markers")
