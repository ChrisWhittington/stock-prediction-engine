...
║    aem_signal_chart.png     — candlestick chart with buy/sell markers      ║
...
def run_backtest(pred_df, bcfg, verbose=True):
    ...
    if trend_on:
        try:
            ...
            gdx_hist = yf.Ticker("AEM").history(
...
def run_benchmark(pred_df, bcfg):
    """Honest Buy-and-Hold equity curve from actual AEM prices.
    ...
    try:
        hist = yf.Ticker("AEM").history(start=start, end=end,
                                              auto_adjust=True)
    except Exception as e:
    ...
    if hist is None or len(hist) < 10:
        # Degenerate fallback: flat equity at initial capital
    ...
    # Align to prediction-df dates (use ffill for non-trading days)
    p_aligned = px.reindex(pred_df.index, method="ffill")
    p_aligned = p_aligned.bfill()  # in case leading reindex returned NaN
    p_aligned = p_aligned.astype(float)

    # Equity = capital_0 * (price / price_0)
    equity = (p_aligned / p_aligned.iloc[0]) * capital_0

    # Metrics from honest equity curve
    dr        = np.log(equity / equity.shift(1)).dropna().values
    ann       = 252
    total_ret = (equity.iloc[-1] / equity.iloc[0] - 1) * 100
    cal_years = (pred_df.index[-1] - pred_df.index[0]).days / 365.25
    cal_years = max(cal_years, 0.01)
...
def plot_candle_signal(results_df, pred_df, lookback_years=2):
    ...
    ohlcv      = ticker.history(start=start_date, end=end_date,
                                auto_adjust=True)
    if len(ohlcv) < 10:
        print(f"  ✗ Could not download AEM OHLCV data")
        return
    ...
    path = os.path.join(BACKTEST_DIR, "aem_signal_chart.png")
    plt.savefig(path, dpi=150, bbox_inches="tight", facecolor=DARK)
    plt.close()
...
def main():
    ...
    print(f"    aem_signal_chart.png     — candle chart with buy/sell markers")
