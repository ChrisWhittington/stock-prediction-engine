# test_shel_candidates.ps1
# Tests every candidate in shel_output\Best_Models_PT\candidates\

# Fix encoding
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$env:PYTHONIOENCODING = "utf-8"

$candidatesDir = "shel_output\Best_Models_PT\candidates"
$shelOutputDir = "shel_output"
$resultsFile   = "shel_output\candidate_results.txt"
...
foreach ($candidate in $candidates) {
    ...
    # files. Without this, shel_backtest.py would read stale predictions and
    # produce duplicate metrics. See 2026-05-27 changelog in PROJECT_NOTES.md.
    Get-ChildItem -Path $shelOutputDir -Filter "fold_*.pt" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue
    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $dest = Join-Path $shelOutputDir $csv
    ...
    foreach ($pt in $ptFiles) {
        Copy-Item $pt.FullName -Destination $shelOutputDir -Force
    }

    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $src = Join-Path $candidate.FullName $csv
        if (Test-Path $src) {
            Copy-Item $src -Destination $shelOutputDir -Force
        }
    }

    Write-Host "  Copied $($ptFiles.Count) fold models"

    $output = python -X utf8 shel_backtest.py 2>&1
    $output | Write-Host
