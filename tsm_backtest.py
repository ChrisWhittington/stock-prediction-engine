...
║    tsm_signal_chart.png     — candlestick chart with buy/sell markers      ║
...
def run_backtest(pred_df, bcfg, verbose=True):
    ...
    if trend_on:
        try:
            ...
            gdx_hist = yf.Ticker("TSM").history(
...
def plot_candle_signal(results_df, pred_df, lookback_years=2):
    ...
    ohlcv      = ticker.history(start=start_date, end=end_date,
                                auto_adjust=True)
    if len(ohlcv) < 10:
        print(f"  ✗ Could not download TSM OHLCV data")
        return
    ...
    ax1.set_ylabel("QQQ Price ($)", color=MUTED, fontsize=9)
    ax1.yaxis.set_major_formatter(
        plt.FuncFormatter(lambda x, _: f"${x:.0f}"))
    ax1.set_xlim(-1, n)
    ...
    path = os.path.join(BACKTEST_DIR, "tsm_signal_chart.png")
    plt.savefig(path, dpi=150, bbox_inches="tight", facecolor=DARK)
    plt.close()
...
def main():
    ...
    if pt_files:
        try:
            ...
            if fwd != BCFG["hold_days"]:
                ...
                BCFG["min_signal"]   = 0.002  # keep same
        except Exception:
            pass
    ...
    print(f"    tsm_signal_chart.png     — candle chart with buy/sell markers")
