"""
╔══════════════════════════════════════════════════════════════════════════════╗
║         AURUM·AI  —  gold_miner_trainer.py  (v10)                          ║
║                                                                              ║
║  Trains a neural net to predict 10-day forward returns on GDX               ║
║  (VanEck Gold Miners ETF).                                                  ║
║                                                                              ║
║  DATA                                                                        ║
║    Downloads 18 tickers via yfinance from 2013 + GPR geopolitical index,   ║
║    CFTC COT gold futures positioning, GLD ETF volume, Google Trends.        ║
║                                                                              ║
...
CFG = {
    # Data
    # v10: 20-day horizon — best performing config
    # forward_days=20, lookback=30, purge_days=40
    # 15-day tested and rejected — worse on all metrics vs 20-day
    "target":          "AEM",
    "start_date":      "2010-01-01",
    "end_date":        datetime.today().strftime("%Y-%m-%d"),
    "forward_days":    20,                  # v10: 20-day horizon (best performing)
    "lookback":        30,                  # v10: LSTM sequence length
    "zscore_window":   252,                 # rolling z-score window (1 trading year)

    # Walk-forward CV
    "n_folds":         8,
    "val_months":      3,
    "purge_days":      40,                  # matched to forward_days=20
    "min_train_years": 2,                   # 2010 start gives ample warmup
...
TICKERS = [
    "AEM",          # target — Agnico Eagle Mines (Canadian, no US estate tax)
    "GDX",          # gold miners ETF (kept as macro feature for sector context)
    "GDXJ",         # junior miners
    "GLD",          # gold ETF
    "GC=F",         # gold futures (front month)
    "SI=F",         # silver futures
    "CL=F",         # crude oil (cost proxy + geopolitical barometer)
    "HG=F",         # copper (macro demand)
    "UUP",          # dollar index ETF (DXY proxy)
    "TLT",          # 20Y treasury (rate proxy)
    "^VIX",         # fear index
    "^VIX3M",       # 3-month VIX — for term structure ratio
    "^TNX",         # 10Y yield
    "^MOVE",        # bond market volatility — rate uncertainty proxy
    "USDJPY=X",     # USD/JPY — JPY is safe-haven, falls in geopolitical risk
    "USDCHF=X",     # USD/CHF — CHF is safe-haven, same signal
    "NEM",          # Newmont
    "GOLD",         # Barrick
    "WPM",          # Wheaton
]
...
def download_gpr(start, end):
    ...
    Daily data, free, no API key. Cache: aurum_output/gpr_cache.csv, 7-day expiry.
    GPR updates monthly so 7-day cache is appropriate.
...
def download_cot(start, end):
    ...
    Files: https://www.cftc.gov/files/dea/history/deacot{year}.zip
    Coverage: 1986→present, weekly (Tuesday close)

    Cache: saves to OUT_DIR/cot_cache.csv, refreshes daily.
    """
    # ── Cache check ───────────────────────────────────────────────────────
    cache_path = os.path.join(OUT_DIR, "cot_cache.csv")
    if os.path.exists(cache_path):
        age_days = (datetime.now() - datetime.fromtimestamp(
                    os.path.getmtime(cache_path))).days
        if age_days < 1:
            try:
                ...
                sliced = cached[mask]
                if len(sliced) > 10:
                    print(f"  ✓ COT: cache hit ({age_days}d old, "
                          f"{len(sliced)} rows)")
                    return sliced
            except Exception:
                pass
    ...
    for year in range(start_year, end_year + 1):
        ...
        try:
            ...
            # Show columns on first year for diagnostics
            if year == start_year:
                print(f"    Columns sample: "
                      f"{[c for c in df_yr.columns[:8]]}")
            ...
            if len(filtered) == 0:
                ...
                for col in df_yr.columns:
                    if "gold" in str(df_yr[col].iloc[0] if len(df_yr) > 0
                                    else "").lower():
                        filtered = df_yr
                        break
                if len(filtered) == 0:
                    print(f"    {year}: no gold rows — "
                          f"cols: {list(df_yr.columns[:6])}")
                    continue
    ...
    # Show actual columns for diagnostics
    print(f"  COT raw columns: {list(cot_raw.columns[:10])}")
    ...
    if iso_col in cot_raw.columns:
        ...
        # YYMMDD format e.g. 230103 = 2023-01-03
        cot_raw["date"] = pd.to_datetime(
    ...
    cot_raw = cot_raw.dropna(
        subset=["date"]).set_index("date").sort_index()

    # Map column names — try multiple format variants
    col_map = {
        # Legacy format
        ...
        "Noncommercial Long":                     "spec_long",
        "Noncommercial Short":                    "spec_short",
        "Commercial Long":                        "comm_long",
        "Commercial Short":                       "comm_short",
        "Open Interest":                          "open_int",
    }
    cot_raw = cot_raw.rename(columns=col_map)

    # Fuzzy fallback — try partial matching if exact rename didn't work
    if "spec_long" not in cot_raw.columns:
        for col in cot_raw.columns:
            ...
            elif col == "Open Interest (All)" or \
                    ("open interest" in cl and "all" in cl):
                cot_raw = cot_raw.rename(columns={col: "open_int"})
        print(f"  COT cols after fuzzy match: "
              f"{[c for c in cot_raw.columns if c in ['spec_long','spec_short','comm_long','comm_short','open_int']]}")
    ...
    cot["spec_long"]  = pd.to_numeric(
        cot_raw.get("spec_long"),  errors="coerce")
    cot["spec_short"] = pd.to_numeric(
        cot_raw.get("spec_short"), errors="coerce")
    cot["comm_long"]  = pd.to_numeric(
        cot_raw.get("comm_long"),  errors="coerce")
    cot["comm_short"] = pd.to_numeric(
        cot_raw.get("comm_short"), errors="coerce")
    ...
    cot = cot[["net_spec", "net_comm",
               "spec_oi_ratio", "comm_oi_ratio",
               "open_int"]].dropna(how="all")
    ...
    print(f"  ✓ COT: {len(cot)} weekly rows  "
    ...
    try:
        ...
        print(f"  ✓ COT: cached → {cache_path}")
    except Exception as e:
        print(f"  ⚠ COT cache save failed: {e}")

    # Return sliced to requested range
    mask = (cot_daily.index >= pd.Timestamp(start)) & \
           (cot_daily.index <= pd.Timestamp(end))
    result = cot_daily[mask]
    print(f"  ✓ COT: {len(result)} daily rows  "
          f"net_spec range: "
          f"{result['net_spec'].min():.0f} → {result['net_spec'].max():.0f}")
    return result
...
def download_google_trends(start, end, keywords=None):
    ...
    Cache: saves to OUT_DIR/google_trends_cache_<tag>.csv
    Reuses cache if less than 7 days old — avoids rate limits during seed search.
    Cache is keyword-keyed so works across all forward_days/lookback configs.
    ...
    # ── Cache check ───────────────────────────────────────────────────────
    # Key by sorted keywords so order doesn't matter
    ...
    try:
        ...
        # Download full history for cache (always from 2004 for maximum coverage)
...
def build_features(prices, cfg, gpr=None, cot=None,
    ...
    # ── Reference series (use fallback if primary missing) ────────────────
    # All series normalised via prices index above
    ...
    # ── 2. Ratio features — margin & sentiment proxies ────────────────────
    if gold is not None and oil is not None:
        ratio = gold / (oil + 1e-9)
        df["gold_oil_ratio"]  = rolling_zscore(ratio, zw)

    if gld is not None:
        ratio = gdx / (gld + 1e-9)
        df["gdx_gld_ratio"]   = rolling_zscore(ratio, zw)
        ratio5 = log_return(gdx / gld, 5)
        df["gdx_gld_mom5"]    = rolling_zscore(ratio5, zw)

    if gold is not None:
        gs = gold / (prices.get("SI=F", gold) + 1e-9)
        df["gold_silver_ratio"] = rolling_zscore(gs, zw)

    if gdxj is not None:
        ratio = gdx / (gdxj + 1e-9)
        df["gdx_gdxj_ratio"]  = rolling_zscore(ratio, zw)
    ...
    if cot is not None:
        # Diagnostic — show what columns COT actually has
        print(f"  COT input cols: {list(cot.columns)}  "
              f"index type: {type(cot.index[0])}")
        ...
        # Normalise prices index too
        prices_idx = prices.index.normalize().tz_localize(None) \
                     if prices.index.tz is not None \
                     else prices.index.normalize()
        cot_aligned = cot_norm.shift(3, freq="B").reindex(prices_idx).ffill()  # CFTC 3-biz-day publication lag (PATCHED v8)
        cot_aligned.index = prices.index  # restore original index
        print(f"  COT aligned: {cot_aligned.notna().all(axis=1).sum()} "
              f"complete rows  "
    ...
    # ── 11. Macro Regime Features ─────────────────────────────────────────
    # v9: dollar_trend only — ranked IC=0.101 (11th of 119) consistently.
    # All other macro features (TIPS, gold secular, regime composite) tested
    # and removed — they did not improve walk-forward IC or backtest Sharpe.

    # ── 11a. Dollar secular trend ─────────────────────────────────────────
    # Dollar bear market = gold bull — 50d vs 200d MA of UUP.
    # This feature is new vs v8 and consistently ranks in top 20.
    if uup is not None:
    ...
    # ── 12. GDX momentum features ─────────────────────────────────────────
    # GDX own price momentum — trend persistence in gold mining stocks
    gdx_close = gdx
    gdx_ma50  = gdx_close.rolling(50).mean()
    gdx_ma200 = gdx_close.rolling(200).mean()
    df["gdx_r5"]          = rolling_zscore(log_return(gdx_close, 5),  zw)
    df["gdx_r10"]         = rolling_zscore(log_return(gdx_close, 10), zw)
    df["gdx_r21"]         = rolling_zscore(log_return(gdx_close, 21), zw)
    df["gdx_ma50_dist"]   = rolling_zscore(
        gdx_close / (gdx_ma50 + 1e-9) - 1, zw)
    df["gdx_ma200_dist"]  = rolling_zscore(
        gdx_close / (gdx_ma200 + 1e-9) - 1, zw)
    df["gdx_trend"]       = rolling_zscore(
        gdx_ma50 / (gdx_ma200 + 1e-9) - 1, zw)   # golden/death cross
    print(f"  ✓ GDX momentum features added (6 features)")

    # ── 13. Silver momentum ───────────────────────────────────────────────
    # Silver leads gold in bull markets — silver/gold ratio expanding = bullish
    silver = prices.get("SI=F")
    if silver is not None and gold is not None:
        df["silver_r10"]      = rolling_zscore(log_return(silver, 10), zw)
        df["silver_r21"]      = rolling_zscore(log_return(silver, 21), zw)
        ag_au_ratio = silver / (gold + 1e-9)
        df["silver_gold_ratio"] = rolling_zscore(ag_au_ratio, zw)
        df["silver_gold_mom"]   = rolling_zscore(
            ag_au_ratio / ag_au_ratio.shift(21) - 1, zw)
        print(f"  ✓ Silver momentum features added")

    fwd = cfg["forward_days"]
    # Smoothed target — average of returns at days +17, +18, +19, +20
    # Reduces noise vs point-in-time; hold_days unchanged at 20
    ...
    print(f"  ✓ Smoothed target: avg(+17,+18,+19,+{fwd})")
...
def walk_forward_cv(features_df, targets_s, cfg):
    ...
    for fold_i, (tr_idx, va_idx) in enumerate(splits):
        ...
        EARLY_EXIT_IC     = 0.15   # both must pass to exit early
        EARLY_EXIT_DACC   = 0.60
        ABANDON_FOLD_IC   = -0.10  # either triggers abandonment
        ABANDON_FOLD_DACC = 0.40

        best_fold_ic      = -999.0
        best_fold_score   = -999.0  # combined IC + DirAcc score
        ...
        for start_i in range(MAX_STARTS):
            ...
            # Combined score — IC plus bonus for DirAcc above 50%
            # Prevents high-IC/low-DirAcc starts dominating
            ...
            # After MIN_STARTS — decide whether to do extra starts
...
def main():
    ...
    # 1b. Download GPR geopolitical risk index
    gpr = download_gpr(CFG["start_date"], CFG["end_date"])

    # 1c. Download COT futures positioning (CFTC)
    cot = download_cot(CFG["start_date"], CFG["end_date"])
    ...
    # Separate best_run tracking per config (forward_days + lookback)
    ...
    if ic_mean > prev_ic:
        ...
        # Back up fold models to best_ic subfolder
        ...
        if copied > 0:
            ...
            print(f"  ⚠ Backup failed — no .pt files found at {pt_pattern}")
    else:
    ...
    # Save promising models that meet trading-relevant criteria,
    # independent of the IC-only best_run tracking.
    # Criteria proxy for good backtest performance without running backtest:
    #   ic_mean   > 0.30  — minimum predictive quality
    #   da_mean   > 0.68  — directional accuracy (drives win rate)
    #   ic_std    < 0.20  — consistent across folds (not one lucky fold)
    #   sh_mean   > 3.00  — walk-forward Sharpe (proxy for trading Sharpe)
    #   da_min    > 0.50  — no terrible folds (worst fold still usable)
    ...
    CRIT = {
        ...
        "sh_mean_min":  3.00,
        "da_min_min":   0.45,   # relaxed to increase candidate rate
    }
    ...
    if passes:
        # Save to candidates subfolder — these are worth backtesting
        ...
        print(f"    → Run backtest to confirm trading performance")
    else:
