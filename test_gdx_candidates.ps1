# test_gdx_candidates.ps1
# Tests every candidate in aurum_output\Best_Models_PT\candidates\

[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$env:PYTHONIOENCODING = "utf-8"

$candidatesDir = "aurum_output\Best_Models_PT\candidates"
$gdxOutputDir  = "aurum_output"
$resultsFile   = "aurum_output\candidate_results.txt"
...
foreach ($candidate in $candidates) {
    ...
    # files. Without this, aurum_backtest.py would read stale predictions and
    # produce duplicate metrics. See 2026-05-27 changelog in PROJECT_NOTES.md.
    Get-ChildItem -Path $gdxOutputDir -Filter "fold_*.pt" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue
    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $dest = Join-Path $gdxOutputDir $csv
    ...
    foreach ($pt in $ptFiles) {
        Copy-Item $pt.FullName -Destination $gdxOutputDir -Force
    }

    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $src = Join-Path $candidate.FullName $csv
        if (Test-Path $src) {
            Copy-Item $src -Destination $gdxOutputDir -Force
        }
    }

    Write-Host "  Copied $($ptFiles.Count) fold models"

    $output = python -X utf8 aurum_backtest.py 2>&1
    $output | Write-Host
