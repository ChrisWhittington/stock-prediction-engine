"""
╔══════════════════════════════════════════════════════════════════════════════╗
║         AURUM·AI  —  qqq_trainer.py  (v1)                           ║
║                                                                              ║
║  Trains a neural net to predict 10-day forward returns on QQQ               ║
...
CFG = {
    ...
    "lookback":        30,                  # LSTM sequence length (trading days)
...
TICKERS = [
    "TSM",          # target — Taiwan Semiconductor (Taiwan, no US estate tax)
    "QQQ",          # Nasdaq 100 ETF (kept as macro feature for sector context)
    "SPY",          # S&P 500 — broad market context
    "SMH",          # semiconductor ETF — key AI/tech proxy
    "XLK",          # tech sector ETF
...
def download_gpr(start, end):
    ...
    Daily data, free, no API key. Cache: qqq_output/gpr_cache.csv, 1-day expiry.
    ...
    if os.path.exists(cache_path):
        ...
        if age_days < 7:
            try:
                ...
                print(f"  GPR cache read failed: {e}")
    else:
        print(f"  GPR cache not found at: {cache_path}")
...
def download_google_trends(start, end, keywords=None):
    """
    Download Google Trends data for tech/QQQ-related search terms.
    Cache: saves to qqq_output/google_trends_cache_<tag>.csv
    Reuses cache if less than 7 days old — avoids rate limits during seed search.
    """
    if keywords is None:
        keywords = ["nvidia stock", "interest rates", "recession", "AI stocks"]
...
def build_features(prices, cfg, gpr=None, cot=None,
    ...
    # ── 10c. Oil-Gold Correlation — skipped for QQQ (gold-specific) ─────────
    ...
    # ── 11. COT Positioning — skipped for QQQ (gold futures specific) ───────
    #    Gold COT data has no predictive value for Nasdaq tech stocks.
    #    Crude oil COT is also not relevant for QQQ.

    # ── 12. GLD ETF Holdings — skipped for QQQ (gold-specific flow) ─────────
    #    GLD institutional flows predict gold, not tech. Removed to reduce noise.
    ...
    # ── Dollar secular trend ──────────────────────────────────────────────
    # Strong USD = headwind for QQQ (multinational earnings, EM flows)
    ...
    # ── 15. QQQ own momentum features ────────────────────────────────────
    # QQQ price momentum — tech sector trend persistence.
    # Short windows (5/10/21) catch sprints; long windows (63/126) catch the
    # multi-quarter climbs that QQQ is known for. Acceleration + vol-adjusted
    # variants give the model regime context.
    qqq_close = prices[cfg["target"]] if cfg["target"] in prices.columns else None
    if qqq_close is not None:
        qqq_ma50  = qqq_close.rolling(50).mean()
        qqq_ma200 = qqq_close.rolling(200).mean()
        # Existing short/medium-window momentum
        df["qqq_r5"]         = rolling_zscore(log_return(qqq_close, 5),  zw)
        df["qqq_r10"]        = rolling_zscore(log_return(qqq_close, 10), zw)
        df["qqq_r21"]        = rolling_zscore(log_return(qqq_close, 21), zw)
        df["qqq_ma50_dist"]  = rolling_zscore(
            qqq_close / (qqq_ma50 + 1e-9) - 1, zw)
        df["qqq_ma200_dist"] = rolling_zscore(
            qqq_close / (qqq_ma200 + 1e-9) - 1, zw)
        df["qqq_trend"]      = rolling_zscore(
            qqq_ma50 / (qqq_ma200 + 1e-9) - 1, zw)
        # Long-window momentum — added 2026-05-27 for long-climb capture
        df["qqq_r63"]        = rolling_zscore(log_return(qqq_close, 63),  zw)
        df["qqq_r126"]       = rolling_zscore(log_return(qqq_close, 126), zw)
        # Momentum acceleration — picks up when a trend is gaining/losing pace
        qqq_r21_raw          = log_return(qqq_close, 21)
        df["qqq_r21_chg5"]   = rolling_zscore(qqq_r21_raw.diff(5), zw)
        # Volatility-adjusted 21d momentum — "Sharpe of recent moves"
        qqq_daily_ret        = log_return(qqq_close, 1)
        qqq_vol20            = qqq_daily_ret.rolling(20).std() * np.sqrt(252)
        df["qqq_vol_adj_r21"] = rolling_zscore(
            qqq_r21_raw / (qqq_vol20 + 1e-9), zw)
        print(f"  ✓ QQQ momentum features added (10 features)")

    # ── 15b. XLK tech sector peer momentum — added 2026-05-27 ────────────
    # XLK is the pure tech sector ETF; leads QQQ during sector rotations and
    # provides peer-momentum context, especially during multi-quarter climbs.
    xlk = prices.get("XLK")
    if xlk is not None:
        df["xlk_r10"]   = rolling_zscore(log_return(xlk, 10), zw)
        df["xlk_r21"]   = rolling_zscore(log_return(xlk, 21), zw)
        df["xlk_r63"]   = rolling_zscore(log_return(xlk, 63), zw)
        print(f"  ✓ XLK peer momentum features added (3 features)")

    # ── 14. Earnings season indicator ────────────────────────────────────
    # QQQ is heavily driven by mega-cap tech earnings (AAPL, MSFT, NVDA etc).
    # Earnings season = last 3 weeks of Jan, Apr, Jul, Oct.
    # Pre-earnings drift is well documented for QQQ components.
    idx = prices.index
    month      = pd.Series(idx.month, index=idx)
    day        = pd.Series(idx.day,   index=idx)
    # Earnings months: Jan=1, Apr=4, Jul=7, Oct=10 — last 3 weeks = day >= 10
    in_earnings = ((month.isin([1, 4, 7, 10])) & (day >= 10)).astype(float)
    # Also flag the week before earnings (anticipation drift)
    pre_earnings = (
        (month.isin([12, 3, 6, 9])) & (day >= 20)
    ).astype(float)
    df["earnings_season"]  = in_earnings
    df["pre_earnings"]     = pre_earnings
    # Combined: how far into earnings season (ramps up)
    df["earnings_phase"]   = rolling_zscore(
        in_earnings.rolling(5).sum(), zw)
    print(f"  ✓ Earnings season features added  (3 features)")
...
def main():
    ...
    # 1b. GPR — geopolitical risk relevant for QQQ (risk-off affects tech)
    gpr = download_gpr(CFG["start_date"], CFG["end_date"])
    cot = None   # gold COT not relevant for QQQ

    # 1d. Download GLD ETF holdings (institutional flow)
    gld_holdings = None  # GLD holdings not used for QQQ — gold-specific
    ...
    if ic_mean > prev_ic:
        ...
        print(f"    Copy qqq_output\\fold_*.pt from a backup if available")
    ...
    CRIT = {
        "ic_mean_min":  0.20,   # QQQ is harder to predict than GDX
        "da_mean_min":  0.58,
