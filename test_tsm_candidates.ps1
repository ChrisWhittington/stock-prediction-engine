# test_tsm_candidates.ps1
# Tests every candidate in tsm_output\Best_Models_PT\candidates\

# Fix encoding
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$env:PYTHONIOENCODING = "utf-8"

$candidatesDir = "tsm_output\Best_Models_PT\candidates"
$tsmOutputDir  = "tsm_output"
$resultsFile   = "tsm_output\candidate_results.txt"
...
foreach ($candidate in $candidates) {
    ...
    # files. Without this, tsm_backtest.py would read stale predictions and
    # produce duplicate metrics. See 2026-05-27 changelog in PROJECT_NOTES.md.
    Get-ChildItem -Path $tsmOutputDir -Filter "fold_*.pt" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue
    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $dest = Join-Path $tsmOutputDir $csv
    ...
    foreach ($pt in $ptFiles) {
        Copy-Item $pt.FullName -Destination $tsmOutputDir -Force
    }

    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $src = Join-Path $candidate.FullName $csv
        if (Test-Path $src) {
            Copy-Item $src -Destination $tsmOutputDir -Force
        }
    }

    Write-Host "  Copied $($ptFiles.Count) fold models"

    $output = python -X utf8 tsm_backtest.py 2>&1
    $output | Write-Host
