# test_xle_candidates.ps1
# Tests every candidate in xle_output\Best_Models_PT\candidates\

# Fix encoding
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$env:PYTHONIOENCODING = "utf-8"

$candidatesDir = "xle_output\Best_Models_PT\candidates"
$xleOutputDir  = "xle_output"
$resultsFile   = "xle_output\candidate_results.txt"
...
foreach ($candidate in $candidates) {
    ...
    # files. Without this, xle_backtest.py would read stale predictions and
    # produce duplicate metrics — that's what happened to seed 83413 in the
    # 2026-05-27 sweep.
    Get-ChildItem -Path $xleOutputDir -Filter "fold_*.pt" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue
    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $dest = Join-Path $xleOutputDir $csv
    ...
    foreach ($pt in $ptFiles) {
        Copy-Item $pt.FullName -Destination $xleOutputDir -Force
    }

    foreach ($csv in @("oof_predictions.csv", "full_predictions.csv")) {
        $src = Join-Path $candidate.FullName $csv
        if (Test-Path $src) {
            Copy-Item $src -Destination $xleOutputDir -Force
        }
    }

    Write-Host "  Copied $($ptFiles.Count) fold models"

    $output = python -X utf8 xle_backtest.py 2>&1
    $output | Write-Host
